/** as 代码 */
	...
	private function init():void{
		...
		EvtDispacther.instance.addEventListener(EvtDispacther.HERO_BUFF_CHANGE,heroBuffChangeHandle);
	...
	private function heroBuffChangeHandle(event:MsgEvent):void{
		if(this.curHero!=null){
			if(HeroObj(event.data).heroInfo.id==this.curHero.heroInfo.id){
				reset();
			} 
		}
	}
	...
	public function set curHeroObj(heroObj:HeroObj):void{
		...
		this.castleId = curHero.castleObj.cityId;
		curHeroStatus = heroObj.heroStatus;
		curHeroExp=this.curHero.heroExpNeetRate;
		reset();
...
//		this.heroNameText.editable=false;
	...
	private function save():void{
		...
		ControllerFactory.getInstance().getHeroController().addPoints(castleId,curHero.heroInfo.id,(politicsPoint-curHero.heroInfo.manages-curHero.heroInfo.managesAdd),(attackPoint-curHero.heroInfo.powers-curHero.heroInfo.powersAdd),(intelligencePoint-curHero.heroInfo.stratagems-curHero.heroInfo.stratagemsAdd),onAddPointResponse);
		//加点响应
	...
	private function washPoint():void{
		...
		function sureToBuy(isSure:Boolean):void{
			if(isSure){
				if(GameContext.instance.getPlayerObj().itemManager.ifEnoughGold(HeroManConstants.RESET_HERO_POINT_ITEM,Math.ceil(curHero.heroInfo.level/10))){
					ControllerFactory.getInstance().getHeroController().resetHeroPoints(castleId,curHero.heroInfo.id,resetPointResponse);
				}
				else{
		...
		/** 洗点重置响应*/ 
		function resetPointResponse(event:CommonResponse):void{
			if(event.isSuccess()){
				reset();
			}
			else{
	...
	private function onUpgrade():void{
		...
		ControllerFactory.getInstance().getHeroController().reachLevel(castleId,curHero.heroInfo.id,upgradeResponse);
		 //升级响应
...
//				MyAlert.show(event.errorMsg);
	...
	private function onDismiss():void{
		...
		function sureToDis(isTrue:Boolean):void{
			if(isTrue){
				ControllerFactory.getInstance().getHeroController().kickHero(castleId,curHero.heroInfo.id,onDismissResponse);
			}
	...
	private function onSetFree():void{
		ControllerFactory.getInstance().getHeroController().treatSeizedHero(castleId,curHero.heroInfo.id ,0 , onTreatResponse);
	...
	private function onPersuade():void{
		...
		if(curHero.heroInfo.level>(tId+1)*10){
	...
	private function reset():void{
 		if(curHero != null){
			...
			remainPoint = curHero.heroInfo.remains;
//			canUpgrade = curHero.heroInfo.experience > Math.pow(curHero.heroInfo.level,2)*100?true:false;
	...
	private function addExp(event:MouseEvent):void{
		...
		function onAddExp(iRule:ItemRule):void{
			...
			ControllerFactory.getInstance().getHeroController().awardItem(castleId,curHero.heroInfo.id,iRule.itemId,onUseItemResponse);
			/**使用物品响应 */ 
		} 					
		function onUseItemResponse(event:CommonResponse):void{
			if(event.isSuccess()){
	...
	private function addPhisical():void{
		...
		function onPhisicalSelect(i:ItemRule):void{
			ControllerFactory.getInstance().getHeroController().awardItem(castleId,curHero.heroInfo.id,i.itemId,onPhisicalItemUse);
		}
	...
	private function onAwardJewelry(iRule:ItemRule):void{
		if(iRule == null){
			ControllerFactory.getInstance().getHeroController().awardMoney(castleId,curHero.heroInfo.id,onAwardGoldResponse);
		}else{
			ControllerFactory.getInstance().getHeroController().awardItem(castleId,curHero.heroInfo.id,iRule.itemId,onAwardJewelryResponse);}		
...
//			heroNameText.alpha=0;
			...
			ControllerFactory.getInstance().getHeroController().reName(castleId,curHero.heroInfo.id,heroNameText.getTextInputText(),onRenameResponse);
		}
		/**改名响应*/ 
   ...
   private function textChange(event:Event):void{
	...
	private function addLeaderShip():void{
		HeroHelper.instance.addArmyLimit(this.curHero);
	}
